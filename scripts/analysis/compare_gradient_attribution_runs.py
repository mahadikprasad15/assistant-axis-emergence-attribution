...
def axis_scores(row: dict[str, Any]) -> dict[str, float]:
...
def main() -> int:
    ...
    common_axes = set(axis_scores(reference[sample_ids[0]])) & set(axis_scores(candidate[sample_ids[0]]))
    ...
    for name in selected_axes:
        ref_values = [axis_scores(reference[sample_id])[name] for sample_id in sample_ids]
        candidate_values = [axis_scores(candidate[sample_id])[name] for sample_id in sample_ids]
